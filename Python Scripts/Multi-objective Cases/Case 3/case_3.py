...
import pandas as pd
from get_coefficients import ref_area, viscous_drag_estimate, Re_M5,Re_M8,L,v_M5,v_M8,rho_M5,rho_M8
...
def fitness_func(ga_instance,solution,solution_idx):
        ...
        global check_constraint,viscous_drag_estimate
        global v_M5,v_M8,rho_M5,rho_M8, ref_area,L
        ...
        if check_constraint(X1,X2) :
            ...
            s_wet=float(s_wet_surrogate.predict_values(solution))
            # get drag
            D_M5=float(D_M5_surrogate.predict_values(solution))+0.5*rho_M5*(v_M5**2)*ref_area*viscous_drag_estimate(Mach=5,Re_nb=Re_M5,Sref=ref_area,Swet=s_wet/2,Lref=1,version='Stratofly')
            D_M8=float(D_M8_surrogate.predict_values(solution))+0.5*rho_M8*(v_M8**2)*ref_area*viscous_drag_estimate(Mach=8,Re_nb=Re_M8,Sref=ref_area,Swet=s_wet/2,Lref=1,version='Stratofly')
...
for i in range(1,10+1):
    ...
    notification.notify(
        title='CASE 3',
        message=f'Optimisation Run {i} Done',
        app_name='VS Code',
        timeout=10  # seconds
    )
    ...
    visc_M5=np.array([0.5 * rho_M5 * (v_M5**2) * ref_area * viscous_drag_estimate(Mach=5, Re_nb=Re_M5, Sref=ref_area, Swet=swet/2, Lref=1, version='Stratofly') for swet in s_wet])
    visc_M8=np.array([0.5 * rho_M8 * (v_M8**2) * ref_area * viscous_drag_estimate(Mach=8, Re_nb=Re_M8, Sref=ref_area, Swet=swet/2, Lref=1, version='Stratofly') for swet in s_wet])
    pareto_df['D_M5_visc'] = np.array(D_M5) + visc_M5
    pareto_df['D_M8_visc'] = np.array(D_M8) + visc_M8
    ...
    with pd.ExcelWriter(f'{dir}_new.xlsx', engine='openpyxl', mode='a', if_sheet_exists='new') as writer:
